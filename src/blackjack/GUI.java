...
public class GUI {
  ...
  public static Game_GUI newGame = new Game_GUI(gameFrame); // we initialize a 'Game_GUI' in order to control, start,
                                                            // and calculate the blackjack game.
  ...
  public void runGUI(Card[] card_deck, Card[] player1hand, Card[] player2hand, Card[] player3hand, Card[] dealerhand) {
    ...
    gameCheckThread = new Thread(new Runnable() {
      ...
      public Runnable init(Card[] card_deck, Card[] player1hand, Card[] player2hand, Card[] player3hand,
          Card[] dealerhand) {
        try {
      ...
      public void run() {
        ...
        newGame.formGame(this.deck, this.player1, this.player2, this.player3, this.dealer); // we set the atmosphere of
                                                                                            // the game(which is
  ...
  public static Thread gameRefreshThread = new Thread() { // this first thread (each thread must have a run method)
                                                          // serves to continuously [since we put true inside the while
                                                          // loop, it will continue forever.] refresh the component.
    public void run() {
      while (true) {
